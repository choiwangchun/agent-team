...
const WORKFLOW_OPENCLAW_BRIDGE_ENABLED =
  !isServerlessRuntime &&
  String(process.env.WORKFLOW_OPENCLAW_BRIDGE_ENABLED || "true")
    .trim()
    .toLowerCase() !== "false";
const WORKFLOW_OPENCLAW_BIN =
  String(process.env.WORKFLOW_OPENCLAW_BIN || "openclaw").trim() || "openclaw";
const WORKFLOW_OPENCLAW_DEFAULT_AGENT_ID =
  String(process.env.WORKFLOW_OPENCLAW_DEFAULT_AGENT_ID || "main").trim() ||
  "main";
const WORKFLOW_OPENCLAW_DEFAULT_PROVIDER =
  String(process.env.WORKFLOW_OPENCLAW_DEFAULT_PROVIDER || "openai-codex").trim() ||
  "openai-codex";
const WORKFLOW_OPENCLAW_DEFAULT_MODEL =
  String(process.env.WORKFLOW_OPENCLAW_DEFAULT_MODEL || "gpt-5.3-codex").trim() ||
  "gpt-5.3-codex";
const WORKFLOW_OPENCLAW_USE_LOCAL =
  String(process.env.WORKFLOW_OPENCLAW_USE_LOCAL || "false")
    .trim()
    .toLowerCase() === "true";
const WORKFLOW_OPENCLAW_TIMEOUT_SEC = Math.max(
  10,
  Number(
    process.env.WORKFLOW_OPENCLAW_TIMEOUT_SEC ||
      Math.ceil(DATA_AI_MODEL_TIMEOUT_MS / 1000)
  )
);
const WORKFLOW_OPENCLAW_STDIO_MAX_BYTES = Math.max(
  1024 * 1024,
  Number(process.env.WORKFLOW_OPENCLAW_STDIO_MAX_BYTES || 16 * 1024 * 1024)
);
const OPENCLAW_AUTH_SYNC_TTL_MS = Math.max(
  1000,
  Number(process.env.OPENCLAW_AUTH_SYNC_TTL_MS || 15000)
);
const OPENCLAW_AUTH_SYNC_TIMEOUT_MS = Math.max(
  1000,
  Number(process.env.OPENCLAW_AUTH_SYNC_TIMEOUT_MS || 15000)
);
const OPENCLAW_AUTH_SYNC_MAX_BYTES = Math.max(
  256 * 1024,
  Number(process.env.OPENCLAW_AUTH_SYNC_MAX_BYTES || 2 * 1024 * 1024)
);
const OPENCLAW_AGENT_IMPORT_ENABLED =
  String(process.env.OPENCLAW_AGENT_IMPORT_ENABLED || "true")
    .trim()
    .toLowerCase() !== "false";
const OPENCLAW_AGENT_IMPORT_TTL_MS = Math.max(
  1000,
  Number(process.env.OPENCLAW_AGENT_IMPORT_TTL_MS || 10000)
);
const OPENCLAW_AGENT_CONFIG_PATH = String(
  process.env.OPENCLAW_AGENT_CONFIG_PATH || ""
).trim();
...
const initialDashboardData = Object.freeze({
  pageTitle: "kairo Agent Control Center",
...
const OPENCLAW_TOOL_NAME_ALIASES = Object.freeze({
  bash: "exec",
  "apply-patch": "apply_patch",
});
const OPENCLAW_TOOL_GROUPS = Object.freeze({
...
const OPENCLAW_TOOL_PROFILES = Object.freeze({
  minimal: Object.freeze({
    allow: Object.freeze(["session_status"]),
  }),
...
const OPENCLAW_KNOWN_TOOL_IDS = Object.freeze([
  "read",
  "write",
  "edit",
  "apply_patch",
...
const OPENCLAW_TOOL_METADATA = Object.freeze({
  read: Object.freeze({
    label: "read",
  ...
  edit: Object.freeze({
    label: "edit",
    description: "Apply precise in-file edits",
    category: "files",
  }),
  apply_patch: Object.freeze({
    label: "apply_patch",
    description: "Patch files with unified patch format",
    category: "files",
  }),
  exec: Object.freeze({
    label: "exec",
  ...
  memory_search: Object.freeze({
    label: "memory_search",
    description: "Search memory context",
    category: "memory",
  }),
  memory_get: Object.freeze({
    label: "memory_get",
    description: "Load memory item",
    category: "memory",
  }),
  sessions_list: Object.freeze({
    label: "sessions_list",
    description: "List sessions",
    category: "sessions",
  }),
  sessions_history: Object.freeze({
    label: "sessions_history",
    description: "Read session history",
    category: "sessions",
  }),
  sessions_send: Object.freeze({
    label: "sessions_send",
    description: "Send message to session",
    category: "sessions",
  }),
  sessions_spawn: Object.freeze({
  ...
  gateway: Object.freeze({
    label: "gateway",
    description: "Gateway and integrations",
    category: "coordination",
  }),
  nodes: Object.freeze({
    label: "nodes",
...
const OPENCLAW_TOOL_PROFILE_LABELS = Object.freeze({
  minimal: "Minimal",
  coding: "Coding",
  messaging: "Messaging",
  full: "Full",
});
const OPENCLAW_AGENT_PROFILE_FILE_NAMES = Object.freeze({
  agents: "AGENTS.md",
  soul: "SOUL.md",
...
const AGENT_ABOUT_MAX_CHARS = Math.max(
  200,
  Number(process.env.AGENT_ABOUT_MAX_CHARS || 4000)
);
...
const openclawAuthSyncCache = {
  fetchedAt: 0,
  snapshot: null,
  inflight: null,
};
const openclawAgentImportCache = {
  fetchedAt: 0,
  snapshot: null,
  inflight: null,
};
let syncOpenClawAgentsPromise = null;

function invalidateOpenClawAgentImportCache() {
  openclawAgentImportCache.fetchedAt = 0;
  openclawAgentImportCache.snapshot = null;
  openclawAgentImportCache.inflight = null;
  syncOpenClawAgentsPromise = null;
}
...
function buildOpenClawAgentIdFromName(name) {
  return String(name || "")
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
...
function resolveOpenClawAgentIdInput({
  openclawAgentId,
  name,
  existingOpenclawAgentId,
}) {
  if (isMainOpenClawAgentId(existingOpenclawAgentId)) {
    return "main";
  }
  const provided = String(openclawAgentId || "").trim().slice(0, 120);
  if (provided) {
    return provided;
  }
  const fromName = buildOpenClawAgentIdFromName(name);
  return fromName || null;
}

function normalizeOpenClawToolName(name) {
  const normalized = String(name || "").trim().toLowerCase();
  if (!normalized) {
    return "";
  }
  return OPENCLAW_TOOL_NAME_ALIASES[normalized] || normalized;
}

function normalizeOpenClawToolList(list) {
  ...
  return [
    ...new Set(
      list
        .map((item) => normalizeOpenClawToolName(item))
        .filter(Boolean)
    ),
  ];
}

function expandOpenClawToolGroups(list) {
  const normalized = normalizeOpenClawToolList(list);
  const expanded = [];
  for (const token of normalized) {
    const group = OPENCLAW_TOOL_GROUPS[token];
  ...
  return [...new Set(expanded.map((item) => normalizeOpenClawToolName(item)).filter(Boolean))];
}

function compileOpenClawToolPattern(pattern) {
  const normalized = normalizeOpenClawToolName(pattern);
  if (!normalized) {
    return null;
  }
  if (normalized === "*") {
  ...
  if (normalized.includes("*")) {
    return {
      kind: "regex",
      value: new RegExp(
        `^${escapeRegexPattern(normalized).replaceAll("\\*", ".*")}$`
      ),
    };
  }
...
function compileOpenClawToolPatterns(patterns) {
  const expanded = expandOpenClawToolGroups(patterns);
  return expanded
    .map((item) => compileOpenClawToolPattern(item))
    .filter(Boolean);
}

function matchesOpenClawToolPattern(name, compiledPatterns) {
  const normalized = normalizeOpenClawToolName(name);
  for (const pattern of Array.isArray(compiledPatterns) ? compiledPatterns : []) {
    ...
    if (pattern.kind === "exact" && normalized === pattern.value) {
      return true;
    }
    if (
      pattern.kind === "regex" &&
      pattern.value instanceof RegExp &&
      pattern.value.test(normalized)
    ) {
      return true;
    }
  }
...
function matchesOpenClawToolList(name, list) {
  const compiled = compileOpenClawToolPatterns(list);
  const normalized = normalizeOpenClawToolName(name);
  if (matchesOpenClawToolPattern(normalized, compiled)) {
    return true;
  }
  return (
    normalized === "apply_patch" &&
    matchesOpenClawToolPattern("exec", compiled)
  );
}

function isOpenClawToolAllowedByPolicy(name, policy = undefined) {
  const normalized = normalizeOpenClawToolName(name);
  if (!policy || typeof policy !== "object") {
    return true;
  }

  if (matchesOpenClawToolList(normalized, policy.deny)) {
    return false;
  }

  const allow = normalizeOpenClawToolList(policy.allow);
  if (allow.length === 0) {
    return true;
  }
  if (matchesOpenClawToolList(normalized, allow)) {
    return true;
  }
  return normalized === "apply_patch" && matchesOpenClawToolList("exec", allow);
}

function resolveOpenClawToolProfilePolicy(profile) {
  const key = String(profile || "").trim().toLowerCase();
  if (!key || key === "full") {
    return undefined;
  }
  const resolved = OPENCLAW_TOOL_PROFILES[key];
  if (!resolved || typeof resolved !== "object") {
    return undefined;
  }
  const allow = normalizeOpenClawToolList(resolved.allow);
  const deny = normalizeOpenClawToolList(resolved.deny);
...
function normalizeOpenClawToolsConfig(input) {
  ...
  return {
    profile: String(input.profile || "").trim().toLowerCase(),
    allow: normalizeOpenClawToolList(input.allow),
    alsoAllow: normalizeOpenClawToolList(input.alsoAllow),
    deny: normalizeOpenClawToolList(input.deny),
  };
}

function sanitizeOpenClawToolsConfigForWrite(input) {
  const source = input && typeof input === "object" && !Array.isArray(input) ? input : {};
  const normalized = normalizeOpenClawToolsConfig(source);
  const next = {};

  if (normalized.profile) {
    next.profile = normalized.profile;
  }
  if (Object.prototype.hasOwnProperty.call(source, "allow") || normalized.allow.length > 0) {
    next.allow = normalized.allow;
  }
  if (
    Object.prototype.hasOwnProperty.call(source, "alsoAllow") ||
    normalized.alsoAllow.length > 0
  ) {
    next.alsoAllow = normalized.alsoAllow;
  }
  if (Object.prototype.hasOwnProperty.call(source, "deny") || normalized.deny.length > 0) {
    next.deny = normalized.deny;
  }

  return next;
}

function hasExplicitOpenClawToolsConfig(input) {
  const normalized = normalizeOpenClawToolsConfig(input);
  return (
    Boolean(normalized.profile) ||
    normalized.allow.length > 0 ||
    normalized.alsoAllow.length > 0 ||
    normalized.deny.length > 0
  );
}

function deriveOpenClawEffectiveToolList({
  globalToolsConfig,
  agentToolsConfig,
}) {
  const globalTools = normalizeOpenClawToolsConfig(globalToolsConfig);
  const agentTools = normalizeOpenClawToolsConfig(agentToolsConfig);
  const globalProfilePolicy = resolveOpenClawToolProfilePolicy(globalTools.profile);
  const agentProfilePolicy = resolveOpenClawToolProfilePolicy(
    agentTools.profile || globalTools.profile || "full"
  );
  const hasGlobalAllow = globalTools.allow.length > 0;
  const hasAgentAllow = agentTools.allow.length > 0;

  const allCandidates = new Set(OPENCLAW_KNOWN_TOOL_IDS.map((item) => normalizeOpenClawToolName(item)));
  for (const token of [
    ...expandOpenClawToolGroups(globalTools.allow),
    ...expandOpenClawToolGroups(globalTools.alsoAllow),
    ...expandOpenClawToolGroups(globalTools.deny),
    ...expandOpenClawToolGroups(agentTools.allow),
    ...expandOpenClawToolGroups(agentTools.alsoAllow),
    ...expandOpenClawToolGroups(agentTools.deny),
  ]) {
    if (!token || token === "*" || token.startsWith("group:")) {
      continue;
    }
    allCandidates.add(normalizeOpenClawToolName(token));
  }

  const effective = [];
  for (const toolName of allCandidates) {
    let allowedByGlobal = true;
    if (hasGlobalAllow) {
      allowedByGlobal = matchesOpenClawToolList(toolName, globalTools.allow);
    } else {
      allowedByGlobal = isOpenClawToolAllowedByPolicy(toolName, globalProfilePolicy);
      if (matchesOpenClawToolList(toolName, globalTools.alsoAllow)) {
        allowedByGlobal = true;
      }
    }
    if (matchesOpenClawToolList(toolName, globalTools.deny)) {
      allowedByGlobal = false;
    }

    let allowedByAgent = true;
    if (hasAgentAllow) {
      allowedByAgent = isOpenClawToolAllowedByPolicy(toolName, {
        allow: agentTools.allow,
        deny: agentTools.deny,
      });
    } else {
      allowedByAgent = isOpenClawToolAllowedByPolicy(toolName, agentProfilePolicy);
      if (matchesOpenClawToolList(toolName, agentTools.alsoAllow)) {
        allowedByAgent = true;
      }
      if (matchesOpenClawToolList(toolName, agentTools.deny)) {
        allowedByAgent = false;
      }
    }

    if (allowedByGlobal && allowedByAgent) {
      effective.push(toolName);
    }
  }

  return [...new Set(effective.map((item) => normalizeOpenClawToolName(item)).filter(Boolean))];
}

function buildOpenClawAgentToolsConfigFromSelection(selectedTools) {
  const allow = normalizeOpenClawToolList(selectedTools);
  if (allow.length === 0) {
    return sanitizeOpenClawToolsConfigForWrite({
      allow: [],
      alsoAllow: [],
      deny: ["*"],
    });
...
function upsertOpenClawAgentConfigListEntry(
  list,
  { openclawAgentId, name, workspacePath, tools, model } = {}
) {
  const normalizedOpenclawAgentId = String(openclawAgentId || "").trim();
  const rows = Array.isArray(list) ? list : [];
  const nextList = rows.map((item) =>
    item && typeof item === "object" ? { ...item } : {}
  );

  if (!normalizedOpenclawAgentId) {
    return nextList;
  }

  const matchIndex = nextList.findIndex(
    (item) =>
      normalizeOpenClawAgentMatchToken(item?.id) ===
      normalizeOpenClawAgentMatchToken(normalizedOpenclawAgentId)
  );
  const existing = matchIndex >= 0 ? nextList[matchIndex] : null;
  const nextEntry = {
    ...(existing && typeof existing === "object" ? existing : {}),
    id: normalizedOpenclawAgentId,
    name:
      String(name || normalizedOpenclawAgentId).trim() || normalizedOpenclawAgentId,
    workspace: String(workspacePath || "").trim() || null,
  };
  if (tools !== undefined) {
    nextEntry.tools = buildOpenClawAgentToolsConfigFromSelection(tools);
  } else if (
    nextEntry?.tools &&
    typeof nextEntry.tools === "object" &&
    !Array.isArray(nextEntry.tools)
  ) {
    nextEntry.tools = sanitizeOpenClawToolsConfigForWrite(nextEntry.tools);
  }
  if (model !== undefined) {
    const normalizedModel = String(model || "").trim();
    if (normalizedModel) {
      nextEntry.model = normalizedModel;
    } else {
...
function applyOpenClawDefaultModelForMainAgent(parsedConfig, model) {
  const normalizedModel = String(model || "").trim();
  if (!normalizedModel || !parsedConfig || typeof parsedConfig !== "object") {
    return;
  }

  const existingAgents =
    parsedConfig?.agents && typeof parsedConfig.agents === "object"
      ? parsedConfig.agents
      : {};
  const existingDefaults =
    existingAgents?.defaults && typeof existingAgents.defaults === "object"
      ...
      : {};
  const existingModel =
    existingDefaults?.model && typeof existingDefaults.model === "object"
  ...
  parsedConfig.agents = {
    ...
    defaults: {
      ...
      models: {
        ...existingModels,
        [normalizedModel]:
          existingModels?.[normalizedModel] &&
          typeof existingModels[normalizedModel] === "object"
            ? existingModels[normalizedModel]
            : {},
      },
    },
...
function listOpenClawToolsForProfile(profileId) {
  const normalizedProfile = String(profileId || "").trim().toLowerCase();
  if (!normalizedProfile || normalizedProfile === "full") {
    return [...OPENCLAW_KNOWN_TOOL_IDS];
  }
  const policy = resolveOpenClawToolProfilePolicy(normalizedProfile);
  if (!policy) {
    return [...OPENCLAW_KNOWN_TOOL_IDS];
  }
  return OPENCLAW_KNOWN_TOOL_IDS.filter((toolName) =>
    isOpenClawToolAllowedByPolicy(toolName, policy)
  );
}

function buildOpenClawToolAccessCatalogResponse() {
  const tools = OPENCLAW_KNOWN_TOOL_IDS.map((toolId) => {
    const normalizedId = normalizeOpenClawToolName(toolId);
    const meta = OPENCLAW_TOOL_METADATA[normalizedId] || {};
    return {
      id: normalizedId,
  ...
  const profiles = Object.keys(OPENCLAW_TOOL_PROFILES).map((profileId) => ({
    id: profileId,
    label:
      OPENCLAW_TOOL_PROFILE_LABELS[profileId] ||
      profileId.charAt(0).toUpperCase() + profileId.slice(1),
    tools: listOpenClawToolsForProfile(profileId),
  }));
...
function areStringSetsEqual(left, right) {
  const a = new Set(normalizeStringArray(left).map((item) => item.toLowerCase()));
  const b = new Set(normalizeStringArray(right).map((item) => item.toLowerCase()));
  if (a.size !== b.size) {
    return false;
  }
...
function toAgentApiResponse(agent, options = {}) {
  if (!agent || typeof agent !== "object") {
    return null;
  }
  const defaultOpenclawAgentIds =
    options?.defaultOpenclawAgentIds instanceof Set
      ? options.defaultOpenclawAgentIds
      : null;
  const normalizedOpenclawAgentId = normalizeOpenClawAgentMatchToken(
    agent?.openclawAgentId
  );
  const isMainAgent = isMainOpenClawAgentId(normalizedOpenclawAgentId);
  const isDefault = Boolean(
    isMainAgent ||
      (normalizedOpenclawAgentId &&
        defaultOpenclawAgentIds &&
        defaultOpenclawAgentIds.has(normalizedOpenclawAgentId))
  );
  return {
    ...agent,
    isSystem: isMainAgent,
    isProtected: isMainAgent,
    isDefault,
  };
}
  ...
  const normalizeTaskBase = (item, index) => {
    ...
    const baseInput =
      item?.input && typeof item.input === "object" && !Array.isArray(item.input)
        ? { ...item.input }
        : {};
    baseInput.executionMode = "parallel";
...
function toPublicProviderAuthConnection(connection) {
  ...
  const models = resolveNormalizedConnectionModels(connection);
...
function resolveNormalizedConnectionModels(connection) {
  const provider = normalizeProviderId(connection?.provider);
  const explicitProviderModels = normalizeProviderModels("", connection?.models);
  if (explicitProviderModels.length > 0) {
    return explicitProviderModels;
  }
  return normalizeProviderModels(provider, connection?.models);
}
...
function getModelCatalogFromConnections(connections) {
  ...
  for (const connection of connections) {
    ...
    const normalizedModels = resolveNormalizedConnectionModels(connection);
...
function resolveOpenClawModelSelectionFromModelTier(modelTierRaw) {
  const modelTier = String(modelTierRaw || "").trim();
  if (!modelTier) {
    return "";
  }
  if (modelTier.toLowerCase() === "balanced (default)".toLowerCase()) {
    return "";
  }

  const parsed = parseModelSelectionValue(modelTier);
  ...
  if (value.includes("/")) {
    ...
    const normalizedModelId = rest.join("/").trim();
  ...
  if (!modelId && value) {
    const inferred = value.toLowerCase();
    if (inferred.includes("codex")) {
      return `openai-codex/${value}`;
    }
    if (
      inferred.startsWith("gpt-") ||
      inferred.startsWith("o1") ||
      inferred.startsWith("o3") ||
      inferred.startsWith("o4")
    ) {
      return `openai/${value}`;
    }
  }
...
function sanitizeAgentAboutText(value) {
  return normalizeLongText(value, AGENT_ABOUT_MAX_CHARS);
}

function inferAgentRoleFromAbout(name, aboutText) {
  const source = `${String(name || "")} ${String(aboutText || "")}`.toLowerCase();
  const roles = [
    { pattern: /(ceo|최고경영|대표)/i, role: "CEO" },
    { pattern: /(cto|기술책임|테크 리드|tech lead)/i, role: "CTO" },
    { pattern: /(마케터|marketing|growth|그로스)/i, role: "마케터" },
    { pattern: /(분석가|analyst|research)/i, role: "분석가" },
    { pattern: /(개발|엔지니어|developer|fullstack|풀스택)/i, role: "개발자" },
    { pattern: /(디자인|designer|ux|ui)/i, role: "디자이너" },
  ];
...
function normalizeMarkdownDocContent(value, fallback = "", maxLength = 20000) {
  const text = normalizeLongText(value, maxLength);
  const normalized = String(text || "").replace(/\r\n/g, "\n").trim();
  const next = normalized || String(fallback || "").trim();
  if (!next) {
    return "";
  }
  return `${next}\n`;
...
function buildDefaultHeartbeatMarkdown() {
  return normalizeMarkdownDocContent(
    [
      "# HEARTBEAT.md",
      "",
...
function buildFallbackAgentProfileDocuments({
  agentName,
  aboutText,
  role,
  modelTier,
  ...
  const safeName = String(agentName || "Agent").trim() || "Agent";
  const safeRole = String(role || "전문가").trim() || "전문가";
  const safeAbout =
    truncateSingleLine(aboutText, 1000) ||
    `${safeName}는 목표를 명확히 분해하고 실행 가능한 결과를 만들어내는 에이전트입니다.`;
  const safeModel = String(modelTier || "Balanced (default)").trim() || "Balanced (default)";
  const toolsText = normalizeStringArray(tools).join(", ") || "none";
  const skillsText = normalizeStringArray(skills).join(", ") || "none";
  const safeUser = String(userName || "사용자").trim() || "사용자";

  const agentsMd = normalizeMarkdownDocContent(
    [
      "# AGENTS.md",
      "",
      `## Agent`,
      ...
      "## Working Rules",
      "- 모든 답변은 한국어로 작성한다.",
      "- 주장에는 근거와 가정을 함께 제시한다.",
      "- 결과물은 즉시 실행 가능한 형태(체크리스트/단계/명령)로 정리한다.",
      "- 작업 완료 시 다음 담당자에게 넘길 수 있는 handoff 문장을 포함한다.",
    ].join("\n")
  );

  const soulMd = normalizeMarkdownDocContent(
    [
      "# SOUL.md",
      "",
      `나는 ${safeRole} 역할의 ${safeName}이다.`,
      "- 정밀함과 실행력을 우선한다.",
      "- 추측보다 검증 가능한 사실을 먼저 제시한다.",
      "- 복잡한 문제는 단계별로 분해해 전달한다.",
    ].join("\n")
  );

  const identityMd = normalizeMarkdownDocContent(
    [
      "# IDENTITY.md",
      "",
      `name: ${safeName}`,
      `role: ${safeRole}`,
      "style: concise, pragmatic, evidence-driven",
      "language: ko",
      `model_hint: ${safeModel}`,
    ].join("\n")
  );

  const userMd = normalizeMarkdownDocContent(
    [
      "# USER.md",
      "",
      `name: ${safeUser}`,
      "preferences:",
      "- 한국어로 간결하고 실행 중심 답변 선호",
      "- 의사결정 시 리스크/트레이드오프 명시",
      "- 최종 결과 전에 핵심 요약 제공",
    ].join("\n")
  );

  const toolsMd = normalizeMarkdownDocContent(
    [
      "# TOOLS.md",
      "",
      "## Tool Access",
      ...
      "",
      "## Local Notes",
      "- 여기에 환경 특화 정보(장비 이름, 경로, 호스트, 계정 닉네임)를 기록합니다.",
      "- 비밀값(API Key, 비밀번호, 토큰)은 저장하지 않습니다.",
      "",
      "## Runtime Defaults",
      "- shell: zsh",
      "- os: macOS",
      "",
      "## Collaboration Hints",
      `- 사용자를 "${safeUser}"로 인식하고, 실행 전 위험 작업 여부를 확인합니다.`,
      "- 경로/명령은 복사 가능한 형태로 짧게 제공합니다.",
    ].join("\n")
  );

  const heartbeatMd = buildDefaultHeartbeatMarkdown();

  return {
    role: safeRole,
    summary: `${safeName} 프로필 파일을 기본 템플릿으로 구성했습니다.`,
    documents: {
      agentsMd,
      soulMd,
      identityMd,
...
function buildAgentProfileModelPrompts({
  agentName,
  aboutText,
  modelTier,
  tools,
  ...
  const systemPrompt = [
    "You are an assistant that designs OpenClaw agent profile files.",
    "Return ONLY one JSON object. No markdown fences.",
    "Write all markdown content in Korean.",
    "Required keys: role, summary, agentsMd, soulMd, identityMd, userMd, toolsMd, heartbeatMd.",
    "Each *Md field must be valid markdown text and concise.",
    "agentsMd should include mission, rules, and operating style.",
    "soulMd should capture personality/tone.",
    "identityMd should capture name/role/style/model hint.",
    "userMd should capture user preferences and collaboration style.",
    "toolsMd should capture local setup notes and tool usage hints.",
    "heartbeatMd should use comments only and match HEARTBEAT.md guidance.",
  ].join(" ");

  const payload = {
    agent: {
      name: String(agentName || "").trim(),
      about: String(aboutText || "").trim(),
      model: String(modelTier || "").trim(),
      tools: normalizeStringArray(tools),
      skills: normalizeStringArray(skills),
    },
    user: {
      name: String(userName || "사용자").trim() || "사용자",
    },
    outputSchema: {
      role: "string",
      summary: "string",
      agentsMd: "string markdown",
      soulMd: "string markdown",
      identityMd: "string markdown",
      userMd: "string markdown",
      toolsMd: "string markdown",
      heartbeatMd: "string markdown",
    },
  };

  return {
    systemPrompt,
    userPrompt: `아래 JSON 입력을 바탕으로 파일을 생성하세요.\n${JSON.stringify(payload, null, 2)}`,
  };
}

function parseAgentProfileDocumentsFromModelOutput(rawText, fallback) {
  const parsed = parseJsonObjectLoose(rawText);
  const fallbackRole = String(fallback?.role || "전문가").trim() || "전문가";
  const fallbackDocs = fallback?.documents || {};

  const role = truncateSingleLine(parsed.role || parsed.agentRole || fallbackRole, 120) || fallbackRole;
  const summary =
    truncateSingleLine(parsed.summary || parsed.message || "", 280) ||
    truncateSingleLine(fallback?.summary || "", 280) ||
    "모델이 Agent 프로필 파일을 생성했습니다.";

  return {
    role,
    summary,
    documents: {
      agentsMd: normalizeMarkdownDocContent(parsed.agentsMd, fallbackDocs.agentsMd),
      soulMd: normalizeMarkdownDocContent(parsed.soulMd, fallbackDocs.soulMd),
      identityMd: normalizeMarkdownDocContent(parsed.identityMd, fallbackDocs.identityMd),
      userMd: normalizeMarkdownDocContent(parsed.userMd, fallbackDocs.userMd),
      toolsMd: normalizeMarkdownDocContent(parsed.toolsMd, fallbackDocs.toolsMd),
      heartbeatMd: buildDefaultHeartbeatMarkdown(),
    },
  };
}

function resolveAgentProfileModelSelection(modelTierRaw) {
  const modelTier = String(modelTierRaw || "").trim();
  const parsedModel = parseModelSelectionValue(modelTier);
  let provider = parsedModel.provider;
  let modelId = String(parsedModel.modelId || "").trim();
  ...
  const useOpenClawBridge =
    WORKFLOW_OPENCLAW_BRIDGE_ENABLED && !isServerlessRuntime;
  ...
  if (
    ...
    if (useOpenClawBridge) {
      provider =
        normalizeProviderId(WORKFLOW_OPENCLAW_DEFAULT_PROVIDER) || "openai-codex";
      modelId =
        String(WORKFLOW_OPENCLAW_DEFAULT_MODEL || "").trim() || "gpt-5.3-codex";
    } else {
      provider = "openai-codex";
...
function isOpenClawUnknownAgentIdError(error) {
  const message = String(error?.message || "").toLowerCase();
  return message.includes("unknown agent id");
}

async function generateAgentProfileDocuments({
  agentName,
  aboutText,
  modelTier,
  tools,
  ...
  const role = inferAgentRoleFromAbout(agentName, aboutText);
  const fallback = buildFallbackAgentProfileDocuments({
    agentName,
    aboutText,
    role,
    modelTier,
  ...
  const prompts = buildAgentProfileModelPrompts({
    agentName,
    aboutText,
    modelTier,
    tools,
  ...
  const selection = resolveAgentProfileModelSelection(modelTier);

  try {
    let modelText = "";
    if (selection.useOpenClawBridge) {
      const requestedAgentId = String(openclawAgentId || "").trim();
      const fallbackAgentId = String(
        WORKFLOW_OPENCLAW_DEFAULT_AGENT_ID || "main"
      ).trim() || "main";
      const primaryAgentId = requestedAgentId || fallbackAgentId;
      const runOpenClawProfileBootstrap = async (agentId) => {
        return requestWorkflowTaskResponseFromOpenClaw({
          modelProvider: selection.provider,
          modelId: selection.modelId,
          systemPrompt: prompts.systemPrompt,
          userPrompt: prompts.userPrompt,
          sessionId: "",
          openclawAgentId: String(agentId || "").trim(),
          timeoutSec: WORKFLOW_OPENCLAW_TIMEOUT_SEC,
          agentLabel: String(agentName || "agent").trim(),
          taskTitle: "agent profile bootstrap generation",
        });
      };

      try {
        const openclawResult = await runOpenClawProfileBootstrap(primaryAgentId);
        modelText = String(openclawResult?.text || "").trim();
      } catch (error) {
        const shouldRetryWithDefault =
          isOpenClawUnknownAgentIdError(error) &&
          requestedAgentId &&
          normalizeOpenClawAgentMatchToken(primaryAgentId) !==
            normalizeOpenClawAgentMatchToken(fallbackAgentId);
        if (!shouldRetryWithDefault) {
          throw error;
        }
        const openclawResult = await runOpenClawProfileBootstrap(fallbackAgentId);
        modelText = String(openclawResult?.text || "").trim();
      }
    } else {
      if (!["openai", "openai-codex"].includes(selection.provider)) {
        throw new Error(`지원하지 않는 provider입니다: ${selection.provider}`);
      }
      const connection = await repository.getProviderAuthConnectionByProvider("openai");
      if (!connection || connection.status !== "connected") {
        throw new Error("OPEN AI OAuth 인증이 필요합니다.");
      }
      if (String(connection.authMode || "").trim().toLowerCase() !== "oauth") {
        throw new Error("OPEN AI provider는 oauth 모드여야 합니다.");
      }
      ...
      const candidateModelIds =
        selection.provider === "openai-codex"
          ? buildOpenAICodexFallbackModelIds(selection.modelId, [])
          : [selection.modelId];
      let lastError = null;
      for (const candidateModelId of candidateModelIds) {
        try {
          modelText = await requestWorkflowTaskResponseFromOpenAI({
          ...
          if (modelText) {
            selection.modelId = candidateModelId;
            break;
          }
        } catch (error) {
          lastError = error;
      ...
      if (!modelText) {
        throw lastError || new Error("모델 응답이 비어 있습니다.");
      }
    }

    const parsed = parseAgentProfileDocumentsFromModelOutput(modelText, fallback);
    return {
      role: parsed.role,
    ...
    return {
      ...
      generation: {
        ...
        provider: selection.provider,
        modelId: selection.modelId,
        warning: truncateSingleLine(
          String(error?.message || "agent profile generation failed"),
          220
        ),
      },
    };
...
async function runOpenClawJsonCommand(args, options = {}) {
  const safeArgs = Array.isArray(args) ? args.filter((item) => String(item || "").trim()) : [];
  if (safeArgs.length === 0) {
    return {};
  }
  const timeoutMs = Math.max(
    1000,
    Number(options.timeoutMs || OPENCLAW_AUTH_SYNC_TIMEOUT_MS)
  );
  const maxBuffer = Math.max(
    256 * 1024,
    Number(options.maxBuffer || OPENCLAW_AUTH_SYNC_MAX_BYTES)
  );

  const stdout = await new Promise((resolve, reject) => {
    execFile(
      WORKFLOW_OPENCLAW_BIN,
      safeArgs,
      {
        timeout: timeoutMs,
        maxBuffer,
      ...
      (error, rawStdout, rawStderr) => {
        const output = String(rawStdout || "").trim();
        const stderr = String(rawStderr || "").trim();
        if (error) {
          const parsedError = parseOpenClawCliJson(output);
  ...
  return parseOpenClawCliJson(stdout);
}

function parseOpenClawModelKey(key) {
  ...
  return {
    provider: normalizeProviderId(providerRaw),
    modelId: String(rest.join("/") || "").trim(),
  };
}

function normalizeOpenClawModels(rows) {
  ...
  for (const item of rows) {
    const available =
      item && Object.prototype.hasOwnProperty.call(item, "available")
        ? Boolean(item.available)
        : true;
    if (!available) {
      continue;
    }
    const parsed = parseOpenClawModelKey(item?.key || item?.id || item?.model || "");
    const modelId = String(parsed.modelId || "").trim();
    if (!modelId) {
      continue;
    }
    ...
    const value = `${normalizedProvider}/${modelId}`;
    if (dedupe.has(value)) {
      continue;
    }
    dedupe.add(value);
    const label = String(item?.name || modelId).trim() || modelId;
    models.push({
      provider: normalizedProvider,
      id: modelId,
      label,
    });
    if (models.length >= PROVIDER_AUTH_MODELS_LIMIT) {
      break;
    }
  }
  return models;
...
async function fetchOpenClawOpenAIAuthSnapshot() {
  const checkedAt = new Date().toISOString();
  if (!WORKFLOW_OPENCLAW_BRIDGE_ENABLED || isServerlessRuntime) {
    return {
      checked: false,
      connected: false,
      models: [],
  ...
  try {
    const statusPayload = await runOpenClawJsonCommand(["models", "status", "--json"]);
    const oauthProviders = Array.isArray(statusPayload?.auth?.oauth?.providers)
      ? statusPayload.auth.oauth.providers
      : [];
    const codexProvider = oauthProviders.find(
      (item) => normalizeProviderId(item?.provider) === "openai-codex"
    );
    const providerStatus = String(codexProvider?.status || "").trim().toLowerCase();
    const connected = providerStatus === "ok";
    if (!connected) {
      return {
        checked: true,
        connected: false,
    ...
    let models = [];
    try {
      const listPayload = await runOpenClawJsonCommand(["models", "list", "--json"]);
      models = normalizeOpenClawModels(Array.isArray(listPayload?.models) ? listPayload.models : []);
    } catch {
      models = [];
    }
    if (models.length === 0) {
    ...
    return {
      ...
      models: [],
      lastCheckedAt: checkedAt,
      errorMessage: truncateSingleLine(
        String(error?.message || "OpenClaw auth probe failed"),
        280
      ),
    };
  }
...
async function getCachedOpenClawOpenAIAuthSnapshot() {
  const now = Date.now();
  if (
    openclawAuthSyncCache.snapshot &&
    now - Number(openclawAuthSyncCache.fetchedAt || 0) < OPENCLAW_AUTH_SYNC_TTL_MS
  ) {
    return openclawAuthSyncCache.snapshot;
  }

  if (openclawAuthSyncCache.inflight) {
    return openclawAuthSyncCache.inflight;
  }

  openclawAuthSyncCache.inflight = (async () => {
    const snapshot = await fetchOpenClawOpenAIAuthSnapshot();
    openclawAuthSyncCache.snapshot = snapshot;
    openclawAuthSyncCache.fetchedAt = Date.now();
    return snapshot;
  })().finally(() => {
    openclawAuthSyncCache.inflight = null;
  });

  return openclawAuthSyncCache.inflight;
}

function mergeOpenClawAuthIntoConnections(connections, snapshot) {
  if (!Array.isArray(connections) || connections.length === 0) {
    return [];
  }
  if (!snapshot || snapshot.checked !== true || snapshot.connected !== true) {
    return connections;
  }
  ...
  return connections.map((item) => {
    ...
    return {
      ...
      models: nextModels,
      errorMessage: null,
      lastCheckedAt: snapshot.lastCheckedAt || item.lastCheckedAt || new Date().toISOString(),
    };
  });
}

function getOpenClawAgentConfigCandidatePaths() {
  const paths = [];
  if (OPENCLAW_AGENT_CONFIG_PATH) {
    paths.push(OPENCLAW_AGENT_CONFIG_PATH);
  }
  paths.push(path.join(process.cwd(), ".openclaw", "openclaw.json"));
  if (process.env.HOME) {
    paths.push(path.join(process.env.HOME, ".openclaw", "openclaw.json"));
  }
  return [...new Set(paths.map((item) => String(item || "").trim()).filter(Boolean))];
}

function normalizeOpenClawAgentMatchToken(value) {
  return String(value || "").trim().toLowerCase();
}

function isMainOpenClawAgentId(value) {
  return normalizeOpenClawAgentMatchToken(value) === "main";
}

function enforceMainDefaultInOpenClawList(list, options = {}) {
  const rows = Array.isArray(list) ? list : [];
  const nextList = rows.map((item) =>
    item && typeof item === "object" ? { ...item } : {}
  );
  const mainEntryFromOptions =
    options?.mainEntry && typeof options.mainEntry === "object"
      ? { ...options.mainEntry }
      : null;

  if (!nextList.some((item) => isMainOpenClawAgentId(item?.id))) {
    nextList.unshift(
      mainEntryFromOptions || {
        id: "main",
        name: "Main",
      }
    );
  ...
  return nextList.map((item) => {
    const isMain = isMainOpenClawAgentId(item?.id);
    return {
      ...item,
      default: isMain,
    };
...
function parseOpenClawAgentsListRows(payload) {
...
function normalizeOpenClawAgentsListPayload(payload, options = {}) {
  const rows = parseOpenClawAgentsListRows(payload);
  const rawToolsByAgentId =
    options?.toolsByAgentId && typeof options.toolsByAgentId === "object"
      ? options.toolsByAgentId
      : {};
  const normalizedToolsByAgentId = new Map();
  for (const [agentIdRaw, tools] of Object.entries(rawToolsByAgentId)) {
    const normalizedAgentId = normalizeOpenClawAgentMatchToken(agentIdRaw);
    if (!normalizedAgentId) {
      continue;
    }
    normalizedToolsByAgentId.set(
      normalizedAgentId,
      normalizeOpenClawToolList(tools)
    );
  }
  const hasDefaultToolsOption = Object.prototype.hasOwnProperty.call(
    options || {},
    "defaultTools"
  );
  const normalizedDefaultTools = hasDefaultToolsOption
    ? normalizeOpenClawToolList(options?.defaultTools)
    : null;
  const dedupe = new Set();
  const entries = [];

  for (const item of rows) {
    const normalizedId = normalizeOpenClawAgentMatchToken(item?.id || item?.agentId || "");
    if (!normalizedId || dedupe.has(normalizedId)) {
      continue;
    }
    ...
    const entryName =
      ...
      (normalizedId === "main" ? "Main" : `OpenClaw ${normalizedId}`);
    const workspace = String(item?.workspace || "").trim() || null;
  ...
  const mainWorkspace = String(options?.mainWorkspace || "").trim() || null;
  const normalizedEntries = enforceMainDefaultInOpenClawList(entries, {
    mainEntry: {
      id: "main",
      name: "Main",
  ...
  return normalizedEntries.map((entry) => {
    const openclawAgentId = String(entry?.id || "").trim();
    const normalizedOpenclawAgentId =
      normalizeOpenClawAgentMatchToken(openclawAgentId);
    const name =
      String(entry?.name || "").trim() ||
      (openclawAgentId === "main"
        ? "Main"
        : `OpenClaw ${openclawAgentId || "agent"}`);
    const hasToolsForAgent = normalizedToolsByAgentId.has(
      normalizedOpenclawAgentId
    );
    const toolsKnown = hasToolsForAgent || hasDefaultToolsOption;
    const resolvedTools = hasToolsForAgent
      ? normalizedToolsByAgentId.get(normalizedOpenclawAgentId)
      : normalizedDefaultTools;
    return {
      openclawAgentId,
      name,
      ...
      tools: toolsKnown
        ? [...new Set((Array.isArray(resolvedTools) ? resolvedTools : []).filter(Boolean))]
        : null,
      skills: toolsKnown
        ? [...new Set((Array.isArray(resolvedTools) ? resolvedTools : []).filter(Boolean))]
        : null,
      toolsKnown,
      toolsExplicit: hasToolsForAgent,
    };
  });
}

function removeAgentFromOpenClawConfigList(list, { agentId, openclawAgentId, name }) {
  const rows = Array.isArray(list) ? list : [];
  const normalizedAgentId = normalizeOpenClawAgentMatchToken(agentId);
  const normalizedOpenclawAgentId = normalizeOpenClawAgentMatchToken(openclawAgentId);
  if (isMainOpenClawAgentId(normalizedAgentId) || isMainOpenClawAgentId(normalizedOpenclawAgentId)) {
    return {
      nextList: enforceMainDefaultInOpenClawList(rows),
      removedCount: 0,
    };
  }
  let removedCount = 0;

  const nextList = rows.filter((item) => {
    const itemId = normalizeOpenClawAgentMatchToken(item?.id);
    if (isMainOpenClawAgentId(itemId)) {
      return true;
    }
    const matches =
      normalizedOpenclawAgentId
        ? itemId === normalizedOpenclawAgentId
        : itemId === normalizedAgentId;
    if (matches) {
      removedCount += 1;
      return false;
    }
  ...
  return {
    nextList: enforceMainDefaultInOpenClawList(nextList),
    removedCount,
  };
}

function resolveWorkspacePathFromConfig(configPath, workspaceValue) {
  const raw = String(workspaceValue || "").trim();
  if (!raw) {
    return "";
  }
...
function collectRemovedOpenClawWorkspacePaths(
  list,
  ...
  for (const item of rows) {
    ...
    const matches = normalizedOpenclawAgentId
      ? itemId === normalizedOpenclawAgentId
      : itemId === normalizedAgentId;
    if (!matches) {
      continue;
    }
    const resolved = resolveWorkspacePathFromConfig(configPath, item?.workspace);
    if (resolved) {
      paths.push(resolved);
    }
  }
...
function syncAgentRemovalToOpenClawConfigs({ agentId, openclawAgentId, name }) {
  const candidatePaths = getOpenClawAgentConfigCandidatePaths().filter((candidate) =>
  ...
  for (const configPath of candidatePaths) {
    try {
      ...
      const currentList = Array.isArray(parsed?.agents?.list) ? parsed.agents.list : [];
      const matchedWorkspacePaths = collectRemovedOpenClawWorkspacePaths(
        currentList,
        {
          agentId,
          openclawAgentId,
      ...
      const { nextList, removedCount: removedInFile } = removeAgentFromOpenClawConfigList(
        currentList,
        {
          agentId,
          openclawAgentId,
      ...
      removedCount += removedInFile;
      for (const workspacePath of matchedWorkspacePaths) {
        workspacePaths.add(workspacePath);
      }
    } catch (error) {
      warnings.push(
        `${path.basename(configPath)}: ${truncateSingleLine(
          String(error?.message || "openclaw config sync failed"),
          180
        )}`
      );
    }
  ...
  if (updatedPaths.length > 0) {
    invalidateOpenClawAgentImportCache();
  }
...
function removeOpenClawWorkspaceDirectories(workspacePaths) {
  const targets = [...new Set((Array.isArray(workspacePaths) ? workspacePaths : []).map((item) =>
    String(item || "").trim()
  ))].filter(Boolean);
  const removedPaths = [];
  const skippedPaths = [];
  ...
  for (const target of targets) {
    try {
      ...
      warnings.push(
        `${path.basename(target)}: ${truncateSingleLine(
          String(error?.message || "workspace removal failed"),
          180
        )}`
      );
    }
...
function resolveOpenClawWorkspaceBasePath(parsedConfig, configPath) {
  const defaultsWorkspaceRaw = String(
    parsedConfig?.agents?.defaults?.workspace || ""
  ).trim();
  if (defaultsWorkspaceRaw) {
    if (path.isAbsolute(defaultsWorkspaceRaw)) {
      return defaultsWorkspaceRaw;
    }
    return path.resolve(path.dirname(configPath), defaultsWorkspaceRaw);
  }
...
function resolveOpenClawAgentWorkspacePath({
  parsedConfig,
  configPath,
  openclawAgentId,
  existingWorkspace,
}) {
  const existing = String(existingWorkspace || "").trim();
  if (existing) {
    if (path.isAbsolute(existing)) {
      return existing;
    }
    return path.resolve(path.dirname(configPath), existing);
  }

  const baseWorkspace = resolveOpenClawWorkspaceBasePath(parsedConfig, configPath);
  const normalizedId = String(openclawAgentId || "")
    .trim()
    .replace(/[^\w.-]+/g, "-")
    .replace(/-+/g, "-")
    .replace(/^-+|-+$/g, "");
  ...
  return `${baseWorkspace}-${normalizedId}`;
}

function ensureOpenClawWorkspaceStateFile(targetDir) {
  const openclawDirPath = path.join(targetDir, ".openclaw");
  fs.mkdirSync(openclawDirPath, { recursive: true });

  const stateFilePath = path.join(openclawDirPath, "workspace-state.json");
  ...
  const versionValue = Number(existing?.version);
  const nextState = {
    version: Number.isFinite(versionValue) && versionValue > 0 ? Math.floor(versionValue) : 1,
    bootstrapSeededAt: String(existing?.bootstrapSeededAt || "").trim() || nowIso,
    onboardingCompletedAt: String(existing?.onboardingCompletedAt || "").trim() || nowIso,
  };

  fs.writeFileSync(stateFilePath, `${JSON.stringify(nextState, null, 2)}\n`, "utf8");
}

function ensureOpenClawWorkspaceGitRepository(targetDir) {
  ...
  try {
    ...
    console.warn(
      `[openclaw] workspace git init failed: ${truncateSingleLine(
...
function ensureOpenClawWorkspaceScaffold(targetDir) {
  ensureOpenClawWorkspaceGitRepository(targetDir);
  ensureOpenClawWorkspaceStateFile(targetDir);
}

function writeAgentProfileWorkspaceFiles(workspaceDir, documents) {
  const targetDir = String(workspaceDir || "").trim();
  if (!targetDir) {
    return;
  }
  fs.mkdirSync(targetDir, { recursive: true });
  ensureOpenClawWorkspaceScaffold(targetDir);

  const docs = documents && typeof documents === "object" ? documents : {};
  const fileMap = [
    {
      fileName: OPENCLAW_AGENT_PROFILE_FILE_NAMES.agents,
      content: normalizeMarkdownDocContent(docs.agentsMd, ""),
    },
    {
      fileName: OPENCLAW_AGENT_PROFILE_FILE_NAMES.soul,
      content: normalizeMarkdownDocContent(docs.soulMd, ""),
    },
    {
      fileName: OPENCLAW_AGENT_PROFILE_FILE_NAMES.identity,
      content: normalizeMarkdownDocContent(docs.identityMd, ""),
    },
    {
      fileName: OPENCLAW_AGENT_PROFILE_FILE_NAMES.user,
      content: normalizeMarkdownDocContent(docs.userMd, ""),
    },
    {
      fileName: OPENCLAW_AGENT_PROFILE_FILE_NAMES.tools,
      content: normalizeMarkdownDocContent(docs.toolsMd, ""),
    },
    {
      fileName: OPENCLAW_AGENT_PROFILE_FILE_NAMES.heartbeat,
      content: buildDefaultHeartbeatMarkdown(),
    },
  ];
...
function buildEmptyOpenClawConfigSkeleton(configPath) {
  const workspace = resolveOpenClawWorkspaceBasePath({}, configPath);
  return {
    agents: {
      defaults: {
        workspace,
...
function syncAgentUpsertToOpenClawConfigs({
  openclawAgentId,
  name,
  tools,
  modelTier,
  ...
  const normalizedOpenclawAgentId = String(openclawAgentId || "").trim();
  const resolvedOpenClawModel = resolveOpenClawModelSelectionFromModelTier(
    modelTier
  );
  if (!normalizedOpenclawAgentId) {
    return {
      ...
      updatedPaths: [],
      workspacePaths: [],
      warnings: ["openclawAgentId가 비어 있어 OpenClaw 동기화를 건너뜀"],
      synced: false,
    };
  }

  let candidatePaths = getOpenClawAgentConfigCandidatePaths().filter((candidate) =>
    fs.existsSync(candidate)
  );
  if (candidatePaths.length === 0) {
    const preferred = getOpenClawAgentConfigCandidatePaths()[0];
    if (preferred) {
      try {
        fs.mkdirSync(path.dirname(preferred), { recursive: true });
        const skeleton = buildEmptyOpenClawConfigSkeleton(preferred);
        fs.writeFileSync(preferred, `${JSON.stringify(skeleton, null, 2)}\n`, "utf8");
        ...
        return {
          ...
          workspacePaths: [],
          warnings: [
            `openclaw config init failed: ${truncateSingleLine(
              String(error?.message || "failed to initialize openclaw config"),
              180
            )}`,
          ],
          synced: false,
  ...
  for (const configPath of candidatePaths) {
    try {
      ...
      try {
        ...
        parsed = buildEmptyOpenClawConfigSkeleton(configPath);
      }
      ...
      const workspacePath = resolveOpenClawAgentWorkspacePath({
        parsedConfig: parsed,
        configPath,
        openclawAgentId: normalizedOpenclawAgentId,
        existingWorkspace: existing?.workspace || "",
      });
      const nextList = upsertOpenClawAgentConfigListEntry(currentList, {
        openclawAgentId: normalizedOpenclawAgentId,
        name:
          String(name || normalizedOpenclawAgentId).trim() ||
          normalizedOpenclawAgentId,
        workspacePath,
        tools,
        model: resolvedOpenClawModel,
      });

      const existingMainEntry =
        nextList.find((item) => isMainOpenClawAgentId(item?.id)) || null;
      const mainWorkspacePath = resolveOpenClawAgentWorkspacePath({
        parsedConfig: parsed,
        configPath,
        openclawAgentId: "main",
        existingWorkspace: existingMainEntry?.workspace || "",
      });
      const normalizedList = enforceMainDefaultInOpenClawList(nextList, {
        mainEntry: {
          ...(existingMainEntry && typeof existingMainEntry === "object"
            ? existingMainEntry
            : {}),
          id: "main",
          name:
            String(existingMainEntry?.name || "").trim() ||
            (normalizedOpenclawAgentId === "main"
              ? String(name || "Main").trim() || "Main"
              : "Main"),
      ...
      parsed.agents = {
        ...(parsed?.agents && typeof parsed.agents === "object" ? parsed.agents : {}),
        defaults: {
          ...(parsed?.agents?.defaults && typeof parsed.agents.defaults === "object"
            ? parsed.agents.defaults
            : {}),
          workspace: resolveOpenClawWorkspaceBasePath(parsed, configPath),
        },
        list: normalizedList,
      };
      if (isMainOpenClawAgentId(normalizedOpenclawAgentId)) {
        applyOpenClawDefaultModelForMainAgent(parsed, resolvedOpenClawModel);
      }
      parsed.meta = {
        ...(parsed?.meta && typeof parsed.meta === "object" ? parsed.meta : {}),
        lastTouchedAt: new Date().toISOString(),
      ...
      fs.writeFileSync(configPath, `${JSON.stringify(parsed, null, 2)}\n`, "utf8");
      writeAgentProfileWorkspaceFiles(workspacePath, documents);
      updatedPaths.push(configPath);
      workspacePaths.push(workspacePath);
    } catch (error) {
      warnings.push(
        `${path.basename(configPath)}: ${truncateSingleLine(
          String(error?.message || "openclaw upsert sync failed"),
          180
        )}`
      );
    }
  ...
  return {
    ...
    warnings,
    synced: updatedPaths.length > 0,
  };
}

function buildDefaultOpenClawWorkspacePath(openclawAgentId) {
  const normalizedId = normalizeOpenClawAgentMatchToken(openclawAgentId);
  const homeDir = String(process.env.HOME || "").trim();
  const openclawRoot = homeDir
    ? path.join(homeDir, ".openclaw")
    : path.join(process.cwd(), ".openclaw");
  if (!normalizedId || normalizedId === "main") {
    return path.join(openclawRoot, "workspace");
  }
  return path.join(openclawRoot, `workspace-${normalizedId}`);
}

function isOpenClawAgentAlreadyExistsError(error) {
  const message = String(error?.message || "").toLowerCase();
  return message.includes("already exists") || message.includes("is reserved");
}

function isOpenClawAgentNotFoundError(error) {
  const message = String(error?.message || "").toLowerCase();
  return message.includes("not found") || message.includes("unknown agent");
}

async function syncAgentUpsertToOpenClawCli({
  openclawAgentId,
  workspacePath,
  modelTier,
}) {
  ...
  if (!normalizedOpenclawAgentId) {
    return {
      ...
      synced: false,
      skipped: true,
      warnings: ["openclawAgentId가 비어 있어 OpenClaw CLI 동기화를 건너뜀"],
    };
  }
  if (isMainOpenClawAgentId(normalizedOpenclawAgentId)) {
    return {
  ...
  if (!WORKFLOW_OPENCLAW_BRIDGE_ENABLED || isServerlessRuntime) {
    return {
      ...
      synced: false,
      skipped: true,
      warnings: ["OpenClaw CLI 브리지 비활성 상태로 add 동기화를 건너뜀"],
    };
  }

  const resolvedWorkspacePath =
    String(workspacePath || "").trim() ||
    buildDefaultOpenClawWorkspacePath(normalizedOpenclawAgentId);
  const args = [
    "agents",
    "add",
    normalizedOpenclawAgentId,
  ...
  const normalizedModelTier = String(modelTier || "").trim();
  if (normalizedModelTier.includes("/")) {
    args.push("--model", normalizedModelTier);
  }

  try {
    await runOpenClawJsonCommand(args, {
      timeoutMs: Math.max(OPENCLAW_AUTH_SYNC_TIMEOUT_MS, 20000),
      maxBuffer: OPENCLAW_AUTH_SYNC_MAX_BYTES,
    });
    invalidateOpenClawAgentImportCache();
    return {
      attempted: true,
    ...
    if (isOpenClawAgentAlreadyExistsError(error)) {
      invalidateOpenClawAgentImportCache();
      return {
        attempted: true,
        synced: true,
    ...
    return {
      ...
      warnings: [
        truncateSingleLine(
          String(error?.message || "openclaw agents add sync failed"),
          220
        ),
      ],
    };
...
async function syncAgentRemovalToOpenClawCli({ openclawAgentId }) {
  const normalizedOpenclawAgentId = String(openclawAgentId || "").trim();
  if (!normalizedOpenclawAgentId) {
    return {
      attempted: false,
      ...
      skipped: true,
      removed: false,
      warnings: ["openclawAgentId가 비어 있어 OpenClaw CLI 삭제 동기화를 건너뜀"],
    };
  }
  if (isMainOpenClawAgentId(normalizedOpenclawAgentId)) {
    return {
  ...
  if (!WORKFLOW_OPENCLAW_BRIDGE_ENABLED || isServerlessRuntime) {
    return {
      ...
      skipped: true,
      removed: false,
      warnings: ["OpenClaw CLI 브리지 비활성 상태로 delete 동기화를 건너뜀"],
    };
  }

  try {
    await runOpenClawJsonCommand(
      ["agents", "delete", normalizedOpenclawAgentId, "--force", "--json"],
      {
        timeoutMs: Math.max(OPENCLAW_AUTH_SYNC_TIMEOUT_MS, 20000),
        maxBuffer: OPENCLAW_AUTH_SYNC_MAX_BYTES,
      }
    ...
    if (isOpenClawAgentNotFoundError(error)) {
      invalidateOpenClawAgentImportCache();
      return {
        attempted: true,
        synced: true,
    ...
    return {
      ...
      warnings: [
        truncateSingleLine(
          String(error?.message || "openclaw agents delete sync failed"),
          220
        ),
      ],
    };
...
async function readOpenClawAgentImportSnapshot() {
  const checkedAt = new Date().toISOString();
  if (!OPENCLAW_AGENT_IMPORT_ENABLED) {
    return {
      checkedAt,
      configPath: null,
      modelTier: "",
  ...
  const configPath = getOpenClawAgentConfigCandidatePaths().find((candidate) =>
    fs.existsSync(candidate)
  );

  // Prefer CLI because it reflects actual OpenClaw runtime state (agents add/delete).
  if (WORKFLOW_OPENCLAW_BRIDGE_ENABLED && !isServerlessRuntime) {
    try {
      const listPayload = await runOpenClawJsonCommand(["agents", "list", "--json"], {
        timeoutMs: OPENCLAW_AUTH_SYNC_TIMEOUT_MS,
        maxBuffer: OPENCLAW_AUTH_SYNC_MAX_BYTES,
      });
      const cliRows = parseOpenClawAgentsListRows(listPayload);
      if (cliRows.length === 0) {
        throw new Error("openclaw agents list returned empty payload");
      }

      let mainWorkspacePath = "";
      let defaultTools = [...OPENCLAW_KNOWN_TOOL_IDS];
      const toolsByAgentId = {};
      if (configPath) {
        try {
          const raw = fs.readFileSync(configPath, "utf8");
          const parsed = JSON.parse(raw);
          const globalToolsConfig = normalizeOpenClawToolsConfig(parsed?.tools);
          const rows = Array.isArray(parsed?.agents?.list) ? parsed.agents.list : [];
          const existingMainEntry = rows.find((item) => isMainOpenClawAgentId(item?.id)) || null;
          mainWorkspacePath = resolveOpenClawAgentWorkspacePath({
            parsedConfig: parsed,
            configPath,
            openclawAgentId: "main",
            existingWorkspace: existingMainEntry?.workspace || "",
          ...
          defaultTools = deriveOpenClawEffectiveToolList({
            globalToolsConfig,
            agentToolsConfig: null,
          });
          for (const row of rows) {
            const normalizedAgentId = normalizeOpenClawAgentMatchToken(row?.id);
            if (!normalizedAgentId) {
              continue;
            }
            if (!hasExplicitOpenClawToolsConfig(row?.tools)) {
              continue;
            }
            toolsByAgentId[normalizedAgentId] = deriveOpenClawEffectiveToolList({
              globalToolsConfig,
              agentToolsConfig: row?.tools,
            });
          }
        } catch {
          mainWorkspacePath = "";
          defaultTools = [...OPENCLAW_KNOWN_TOOL_IDS];
        }
      }

      const agents = normalizeOpenClawAgentsListPayload(listPayload, {
        mainWorkspace: mainWorkspacePath,
        toolsByAgentId,
        defaultTools,
      });
      ...
      // Fall through to config parsing fallback.
    }
  }
  ...
  try {
    const raw = fs.readFileSync(configPath, "utf8");
    const parsed = JSON.parse(raw);
    const defaultModelTier = String(
      parsed?.agents?.defaults?.model?.primary || ""
    ).trim();
    const globalToolsConfig = normalizeOpenClawToolsConfig(parsed?.tools);
    const currentRows = Array.isArray(parsed?.agents?.list) ? parsed.agents.list : [];
    const existingMainEntry =
      currentRows.find((item) => isMainOpenClawAgentId(item?.id)) || null;
    const mainWorkspacePath = resolveOpenClawAgentWorkspacePath({
      parsedConfig: parsed,
      configPath,
      openclawAgentId: "main",
    ...
    const rows = enforceMainDefaultInOpenClawList(currentRows, {
      mainEntry: {
        ...(existingMainEntry && typeof existingMainEntry === "object"
          ? existingMainEntry
          : {}),
        id: "main",
        name: String(existingMainEntry?.name || "").trim() || "Main",
        workspace: mainWorkspacePath,
      },
    });
    const dedupe = new Set();
    const agents = [];
    for (const item of rows) {
      const openclawAgentId = String(item?.id || "").trim();
      if (!openclawAgentId || dedupe.has(openclawAgentId)) {
        continue;
      }
      dedupe.add(openclawAgentId);
      const name =
        String(item?.name || "").trim() || `OpenClaw ${openclawAgentId}`;
      const hasExplicitTools = hasExplicitOpenClawToolsConfig(item?.tools);
      const effectiveTools = deriveOpenClawEffectiveToolList({
        globalToolsConfig,
        agentToolsConfig: item?.tools,
      });
      agents.push({
        openclawAgentId,
        name,
        workspace: String(item?.workspace || "").trim() || null,
        isDefault: Boolean(item?.default),
        tools: effectiveTools,
        toolsKnown: true,
        toolsExplicit: hasExplicitTools,
      });
    }

    let resolvedDefaultAgentId = "";
    for (const agent of agents) {
      if (agent.isDefault === true) {
        resolvedDefaultAgentId = String(agent.openclawAgentId || "").trim();
        if (resolvedDefaultAgentId) {
          break;
        }
      }
    }
    if (!resolvedDefaultAgentId && agents.length > 0) {
      resolvedDefaultAgentId = String(agents[0]?.openclawAgentId || "").trim();
    }
    if (resolvedDefaultAgentId) {
      for (const agent of agents) {
        agent.isDefault =
          normalizeOpenClawAgentMatchToken(agent?.openclawAgentId) ===
          normalizeOpenClawAgentMatchToken(resolvedDefaultAgentId);
      }
    }
    ...
    return {
      ...
      agents: [],
      errorMessage: truncateSingleLine(
        String(error?.message || "openclaw config parse failed"),
        280
      ),
      source: "config",
    };
...
async function getCachedOpenClawAgentImportSnapshot() {
  const now = Date.now();
  if (
    openclawAgentImportCache.snapshot &&
    now - Number(openclawAgentImportCache.fetchedAt || 0) < OPENCLAW_AGENT_IMPORT_TTL_MS
  ) {
    return openclawAgentImportCache.snapshot;
  }

  if (openclawAgentImportCache.inflight) {
    return openclawAgentImportCache.inflight;
  }

  openclawAgentImportCache.inflight = Promise.resolve()
    .then(() => readOpenClawAgentImportSnapshot())
    .then((snapshot) => {
      openclawAgentImportCache.snapshot = snapshot;
      openclawAgentImportCache.fetchedAt = Date.now();
      return snapshot;
    })
    .finally(() => {
      openclawAgentImportCache.inflight = null;
...
function buildOpenClawDefaultAgentIdSet(snapshot) {
  const defaults = new Set(["main"]);
  const rows = Array.isArray(snapshot?.agents) ? snapshot.agents : [];
  for (const item of rows) {
    if (!item || item.isDefault !== true) {
      continue;
    }
    const openclawAgentId = normalizeOpenClawAgentMatchToken(
      item?.openclawAgentId
    );
    if (!openclawAgentId) {
      continue;
    }
    defaults.add(openclawAgentId);
  }
  return defaults;
}

async function getOpenClawDefaultAgentIdSet() {
  if (!OPENCLAW_AGENT_IMPORT_ENABLED) {
    return new Set();
  }
  try {
    const snapshot = await getCachedOpenClawAgentImportSnapshot();
    return buildOpenClawDefaultAgentIdSet(snapshot);
  } catch {
    return new Set();
  }
}

function buildImportedAgentIdFromOpenClaw(openclawAgentId, usedIds) {
  const base = String(openclawAgentId || "")
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "");
  const stem = `openclaw-${base || "agent"}`;
  let candidate = stem;
  let seq = 2;
...
function buildOpenClawImportedSystemPrompt(openclawAgentId) {
  const safeAgentId = String(openclawAgentId || "").trim();
  return `Imported from OpenClaw agent '${safeAgentId}'.`;
}

async function syncAgentsFromOpenClawConfig() {
  if (!OPENCLAW_AGENT_IMPORT_ENABLED) {
    return { imported: 0, updated: 0, skipped: 0, removed: 0 };
  }

  if (syncOpenClawAgentsPromise) {
    return syncOpenClawAgentsPromise;
  }

  syncOpenClawAgentsPromise = (async () => {
    const snapshot = await getCachedOpenClawAgentImportSnapshot();
    ...
    const importedModelTier =
      String(snapshot?.modelTier || "").trim() || "openai-codex/gpt-5.3-codex";
    const existingAgents = await repository.listAgents(500);
    const existingByOpenClawId = new Map();
    const existingByOpenClawIdLower = new Map();
    const existingAgentIds = new Set();
    for (const agent of existingAgents) {
      existingAgentIds.add(String(agent?.id || "").trim());
      const key = String(agent?.openclawAgentId || "").trim();
      if (!key) {
        continue;
      }
      existingByOpenClawId.set(key, agent);
      existingByOpenClawIdLower.set(key.toLowerCase(), agent);
    }
    ...
    let skipped = 0;
    let removed = 0;
    const sourceOpenClawIdSet = new Set();

    for (const source of sourceAgents) {
      const openclawAgentId = String(source?.openclawAgentId || "").trim();
      if (!openclawAgentId) {
        skipped += 1;
        continue;
      }
      const normalizedSourceId = normalizeOpenClawAgentMatchToken(openclawAgentId);
      if (normalizedSourceId) {
        sourceOpenClawIdSet.add(normalizedSourceId);
      }

      const sourceName =
        String(source?.name || "").trim() || `OpenClaw ${openclawAgentId}`;
      const sourceToolsKnown = Array.isArray(source?.tools) || Boolean(source?.toolsKnown);
      const sourceToolsExplicit = Boolean(source?.toolsExplicit);
      const sourceTools = sourceToolsKnown
        ? normalizeOpenClawToolList(source?.tools)
        : [];
      const sourceSkills = sourceToolsKnown ? [...sourceTools] : [];
      const openclawAgentIdLower = openclawAgentId.toLowerCase();
      let target =
        existingByOpenClawId.get(openclawAgentId) ||
        existingByOpenClawIdLower.get(openclawAgentIdLower) ||
        null;

      if (target) {
        const patch = {};
        if (!String(target.openclawAgentId || "").trim()) {
          patch.openclawAgentId = openclawAgentId;
        }
        if (!String(target.name || "").trim()) {
          patch.name = sourceName;
        }
        const currentModel = String(target.modelTier || "").trim();
        if (!currentModel || currentModel.toLowerCase() === "balanced (default)") {
          patch.modelTier = importedModelTier;
        }
        if (!String(target.systemPrompt || "").trim()) {
          patch.systemPrompt = buildOpenClawImportedSystemPrompt(openclawAgentId);
        }
        if (
          sourceToolsKnown &&
          sourceToolsExplicit &&
          (!areStringSetsEqual(target.tools, sourceTools) ||
            !areStringSetsEqual(target.skills, sourceSkills))
        ) {
          patch.tools = sourceTools;
          patch.skills = sourceSkills;
        }
        if (Object.keys(patch).length > 0) {
          const updatedAgent = await repository.updateAgent(target.id, patch);
          if (updatedAgent) {
            updated += 1;
            existingByOpenClawId.set(openclawAgentId, updatedAgent);
            existingByOpenClawIdLower.set(openclawAgentIdLower, updatedAgent);
          }
        } else {
          skipped += 1;
        }
      ...
      const createdId = buildImportedAgentIdFromOpenClaw(
        openclawAgentId,
        existingAgentIds
      );
      const created = await repository.createAgent({
        id: createdId,
        name: sourceName,
        modelTier: importedModelTier,
        systemPrompt: buildOpenClawImportedSystemPrompt(openclawAgentId),
        openclawAgentId,
        tools: sourceTools,
        skills: sourceSkills,
        avatarUrl: null,
      ...
      imported += 1;
      existingByOpenClawId.set(openclawAgentId, created);
      existingByOpenClawIdLower.set(openclawAgentIdLower, created);
    }

    if (String(snapshot?.source || "").trim().toLowerCase() === "cli") {
      for (const existing of existingAgents) {
        const existingId = String(existing?.id || "").trim();
        const existingOpenclawId = normalizeOpenClawAgentMatchToken(
          existing?.openclawAgentId
        );
        if (!existingId || !existingOpenclawId || existingOpenclawId === "main") {
          continue;
        }
        if (sourceOpenClawIdSet.has(existingOpenclawId)) {
          continue;
        }
        const removedAgent = await repository.deleteAgent(existingId);
        if (removedAgent) {
          removed += 1;
        }
      }
...
function parseOpenClawCliJson(text) {
  const raw = String(text || "").trim();
  if (!raw) {
    return {};
  }
  ...
  const parsedLoose = parseJsonObjectLoose(raw);
...
function extractOpenClawPayloadText(payload) {
  ...
  const fromPayloads = payloads
    ...
    .filter(Boolean);
  if (fromPayloads.length > 0) {
    return fromPayloads.join("\n\n");
  }

  return String(
    payload?.result?.text || payload?.message || payload?.summary || ""
  ).trim();
}

function resolveOpenClawResponseModel(payload, fallbackProvider, fallbackModelId) {
  const metaProvider = normalizeProviderId(payload?.result?.meta?.agentMeta?.provider);
  const provider = metaProvider || normalizeProviderId(fallbackProvider) || "openai-codex";
  const modelId =
    String(payload?.result?.meta?.agentMeta?.model || fallbackModelId || "").trim() ||
    String(fallbackModelId || "").trim();
  return {
    provider,
    modelId,
  };
...
function buildOpenClawWorkflowMessage({
  modelProvider,
  modelId,
  systemPrompt,
  userPrompt,
  ...
  const provider = normalizeProviderId(modelProvider) || "openai-codex";
  const requestedModel = String(modelId || "").trim() || "gpt-5.3-codex";
  return [
    `WORKFLOW TASK for agent: ${String(agentLabel || "agent").trim()}`,
    `TASK TITLE: ${String(taskTitle || "task").trim()}`,
    `MODEL HINT: ${provider}/${requestedModel}`,
    "",
    "반드시 다음 JSON 스키마로만 응답하세요:",
    '{"summary":"","deliverable":"","insights":[],"nextActions":[],"status":"completed","commands":[],"messages":[]}',
    "추가 텍스트/마크다운/코드펜스 금지.",
    "",
    "[SYSTEM PROMPT]",
...
async function requestWorkflowTaskResponseFromOpenClaw({
  modelProvider,
  modelId,
  systemPrompt,
  userPrompt,
  ...
  const resolvedAgentId = String(
    openclawAgentId || WORKFLOW_OPENCLAW_DEFAULT_AGENT_ID || ""
  ).trim();
  if (resolvedAgentId) {
    args.push("--agent", resolvedAgentId);
  }

  const resolvedSessionId = String(sessionId || "").trim().slice(0, 200);
  if (resolvedSessionId) {
    args.push("--session-id", resolvedSessionId);
  }

  if (WORKFLOW_OPENCLAW_USE_LOCAL) {
    args.push("--local");
  }

  args.push("--timeout", String(Math.max(10, Number(timeoutSec) || 10)));
  args.push(
    "--message",
    buildOpenClawWorkflowMessage({
      modelProvider,
      modelId,
      systemPrompt,
      userPrompt,
  ...
  const stdout = await new Promise((resolve, reject) => {
    execFile(
      ...
      args,
      {
        timeout: Math.max(
          1000,
          Math.floor(Math.max(10, Number(timeoutSec) || 10) * 1000) + 3000
        ),
        maxBuffer: WORKFLOW_OPENCLAW_STDIO_MAX_BYTES,
      },
      (error, rawStdout, rawStderr) => {
        const output = String(rawStdout || "").trim();
        const stderr = String(rawStderr || "").trim();
        if (error) {
          const parsedErrorPayload = parseOpenClawCliJson(output);
          const parsedErrorText = truncateSingleLine(
            parsedErrorPayload?.error?.message ||
              parsedErrorPayload?.message ||
              parsedErrorPayload?.summary ||
              stderr ||
              error.message ||
              "openclaw execution failed",
            320
          );
          reject(new Error(`OpenClaw 실행 실패: ${parsedErrorText}`));
          return;
        }
        resolve(output);
      }
  ...
  const payload = parseOpenClawCliJson(stdout);
  const status = String(payload?.status || "").trim().toLowerCase();
  if (status && status !== "ok") {
    const errorMessage = truncateSingleLine(
      payload?.error?.message ||
        payload?.message ||
        payload?.summary ||
        "openclaw returned non-ok status",
      320
    );
    throw new Error(`OpenClaw 응답 오류: ${errorMessage}`);
  }

  const text = extractOpenClawPayloadText(payload);
  if (!text) {
    throw new Error("OpenClaw 모델 응답이 비어 있습니다.");
  }

  const resolvedModel = resolveOpenClawResponseModel(
    payload,
    modelProvider,
    modelId
  );
...
function resolveOpenClawAgentIdForTask(task, agent) {
  const input =
  ...
  return String(
    input?.openclawAgentId ||
      input?.openclawAgent ||
      agent?.openclawAgentId ||
      WORKFLOW_OPENCLAW_DEFAULT_AGENT_ID
  )
    .trim()
    .slice(0, 120);
}
...
function isTaskLocalExecutionEnabled(task, agent) {
  ...
  const tools = normalizeStringArray(agent?.tools || []).map((item) =>
    normalizeOpenClawToolName(item)
  );
  if (tools.length === 0) {
    return true;
  }
  ...
  return tools.some((item) => {
    const lower = normalizeOpenClawToolName(item);
    return (
      lower === "exec" ||
      lower === "process" ||
      lower === "shell" ||
...
function resolveTaskPermissionProfile(task, agent = null) {
  const input = task?.input && typeof task.input === "object" ? task.input : {};
  const permissions =
  ...
  const hasExplicitProfile = Boolean(profileNameRaw);
  const hasExplicitAllow =
    Array.isArray(permissions?.allowCommands) ||
    Array.isArray(input?.allowCommands);
  const hasExplicitDeny =
    Array.isArray(permissions?.denyPatterns) ||
    Array.isArray(input?.denyPatterns);

  let mode = profileName;
  if (!hasExplicitProfile && !hasExplicitAllow && !hasExplicitDeny) {
    const tools = normalizeStringArray(agent?.tools || []).map((item) =>
      normalizeOpenClawToolName(item)
    );
    if (tools.length > 0) {
      const hasExec = tools.some((tool) => tool === "exec" || tool === "process");
      const hasWriteTool = tools.some((tool) =>
        ["write", "edit", "apply_patch"].includes(tool)
      );
      if (!hasExec) {
        mode = "read_only";
      } else {
        mode = hasWriteTool ? "full" : "standard";
      }
    }
  }
...
async function ensureWorkflowNodeSession({
  ...
  const permissionProfile = resolveTaskPermissionProfile(task, agent);
  const existing =
...
async function executeWorkflowTaskWithAgent({
  ...
  const sessionPermissionProfile = normalizePermissionProfile(
    nodeSession?.permissionProfile && typeof nodeSession.permissionProfile === "object"
      ? nodeSession.permissionProfile
      : resolveTaskPermissionProfile(task, agent)
  );
  let inboxMessages = [];
  if (WORKFLOW_AGENT_TEAMS_ENABLED) {
  ...
  let requestedModelId = String(parsedModel.modelId || "").trim();
  const hasProviderPrefix = modelTier.includes("/");
  const useOpenClawBridge =
    WORKFLOW_OPENCLAW_BRIDGE_ENABLED && !isServerlessRuntime;
  ...
  if (useOpenClawBridge) {
    const bridgeDefaultProvider =
      normalizeProviderId(WORKFLOW_OPENCLAW_DEFAULT_PROVIDER) || "openai-codex";
    const bridgeDefaultModelId =
      String(WORKFLOW_OPENCLAW_DEFAULT_MODEL || "").trim() || "gpt-5.3-codex";
    if (!modelProvider || modelProvider === "system" || isBalancedDefault) {
      modelProvider = bridgeDefaultProvider;
    }
    if (!requestedModelId || isBalancedDefault) {
      requestedModelId = bridgeDefaultModelId;
    }
  }
  ...
  if (!useOpenClawBridge && !["openai", "openai-codex"].includes(modelProvider)) {
    throw new Error(`unsupported task model provider: ${modelProvider}`);
  }

  const useCodexTransport = modelProvider === "openai-codex";
  let codexDiscoveredModelIds = [];
  let availableOpenAIModelIds = [];
  let modelResolution = useCodexTransport
    ? {
        modelId:
          buildOpenAICodexFallbackModelIds(requestedModelId, [])[0] ||
          requestedModelId,
        changed: false,
      }
    : {
  ...
  if (!useOpenClawBridge) {
    ...
    codexDiscoveredModelIds = normalizeProviderModels("openai", connection?.models)
    ...
    modelResolution = useCodexTransport
      ? {
          modelId:
  ...
  let executionProvider = useOpenClawBridge
    ? String(modelProvider || "openai-codex").trim() || "openai-codex"
    : useCodexTransport
    ? "openai-codex"
    : "openai";
  ...
  const openclawAgentId = resolveOpenClawAgentIdForTask(task, agent);

  const requestModelOnce = async (prompts) => {
    if (useOpenClawBridge) {
      const openclawResult = await requestWorkflowTaskResponseFromOpenClaw({
        modelProvider: executionProvider,
        modelId: modelIdForExecution,
        systemPrompt: prompts.systemPrompt,
        userPrompt: prompts.userPrompt,
        sessionId: String(nodeSession?.sessionKey || "").trim(),
        openclawAgentId,
        timeoutSec: WORKFLOW_OPENCLAW_TIMEOUT_SEC,
        agentLabel,
        taskTitle,
      });
      const nextProvider = normalizeProviderId(openclawResult.modelProvider);
      if (nextProvider) {
        executionProvider = nextProvider;
      }
      const nextModelId = String(openclawResult.modelId || "").trim();
      if (nextModelId && nextModelId !== modelIdForExecution) {
        modelIdForExecution = nextModelId;
        modelAutoResolved = true;
      }
      return openclawResult.text;
    }
  ...
  const result = {
    ...
    kind: taskKind,
    agent: agentLabel,
    model: useOpenClawBridge
      ? `openclaw/${executionProvider}/${modelIdForExecution}`
      : `${executionProvider}/${modelIdForExecution}`,
    modelAutoResolved,
...
async function listProviderAuthConnectionsForResponse() {
  ...
  const openclawSnapshot = await getCachedOpenClawOpenAIAuthSnapshot();
  const mergedConnections = mergeOpenClawAuthIntoConnections(
    connections,
    openclawSnapshot
  );
  return mergedConnections.map((item) => toPublicProviderAuthConnection(item));
}
...
async function ensureInitialized() {
  ...
  if (!initPromise) {
    initPromise = (async () => {
      ...
      await ensureDefaultProviderAuthConnections();
      await syncAgentsFromOpenClawConfig();
...
app.get("/api/openclaw/tool-access", (req, res) => {
  const catalog = buildOpenClawToolAccessCatalogResponse();
...
app.get("/api/agents", async (req, res, next) => {
  try {
    ...
    const [agents, defaultOpenclawAgentIds] = await Promise.all([
      repository.listAgents(limit),
      getOpenClawDefaultAgentIdSet(),
    ]);
    return res.json({
      agents: agents
        .map((agent) =>
          toAgentApiResponse(agent, {
            defaultOpenclawAgentIds,
          })
        )
        .filter(Boolean),
...
app.get("/api/agents/:agentId", async (req, res, next) => {
  try {
    await syncAgentsFromOpenClawConfig();
    const [agent, defaultOpenclawAgentIds] = await Promise.all([
      repository.getAgent(req.params.agentId),
      getOpenClawDefaultAgentIdSet(),
    ]);
    if (!agent) {
      return res.status(404).json({
    ...
    return res.json({
      agent: toAgentApiResponse(agent, {
        defaultOpenclawAgentIds,
      }),
    });
  } catch (error) {
...
app.delete("/api/agents/:agentId", ...requireOperatorRole, async (req, res, next) => {
  try {
    ...
    const existing = await repository.getAgent(agentId);
    if (!existing) {
      return res.status(404).json({
        error: "agent_not_found",
    ...
    if (isMainOpenClawAgentId(existing.openclawAgentId)) {
      return res.status(403).json({
        error: "agent_protected",
        message: "main agent는 삭제할 수 없습니다.",
      });
    }
    ...
    const normalizedOpenclawAgentId = normalizeOpenClawAgentMatchToken(
      removed.openclawAgentId
    );
    let relatedRemovedCount = 0;
    if (normalizedOpenclawAgentId) {
      const seenIds = new Set([String(removed.id || "").trim()]);
      for (let pass = 0; pass < 24; pass += 1) {
        const candidates = await repository.listAgents(500);
        const linkedIds = candidates
          .filter((candidate) => {
            ...
            const candidateOpenclawId = normalizeOpenClawAgentMatchToken(
              candidate?.openclawAgentId
            );
            return candidateOpenclawId === normalizedOpenclawAgentId;
          })
        ...
        for (const linkedId of linkedIds) {
          ...
          const linkedRemoved = await repository.deleteAgent(linkedId);
          if (linkedRemoved) {
            relatedRemovedCount += 1;
            removedInPass += 1;
          }
        }
    ...
    const openclawSync = syncAgentRemovalToOpenClawConfigs({
      agentId: removed.id,
      openclawAgentId: removed.openclawAgentId || normalizedOpenclawAgentId,
      name: removed.name,
    });
    const openclawCliSync = await syncAgentRemovalToOpenClawCli({
      openclawAgentId: removed.openclawAgentId || normalizedOpenclawAgentId,
    });
    const workspaceCleanup = removeOpenClawWorkspaceDirectories(
      openclawSync.workspacePaths
    );
...
app.post("/api/agents", ...requireOperatorRole, async (req, res) => {
  try {
    ...
    const agentAbout = sanitizeAgentAboutText(
      req.body?.agentAbout !== undefined ? req.body?.agentAbout : req.body?.systemPrompt
    );
    ...
    if (!agentAbout) {
      return res.status(400).json({
        error: "invalid_agent",
        message: "agentAbout is required",
      });
    }
    ...
    const openclawAgentId = resolveOpenClawAgentIdInput({
      openclawAgentId: req.body?.openclawAgentId,
      name,
    });
    const avatarUrl = String(req.body?.avatarUrl || "").trim() || null;
    const profileDocs = await generateAgentProfileDocuments({
      agentName: name,
      aboutText: agentAbout,
      modelTier,
      tools,
      skills,
      userName: req.authUser?.name || req.authUser?.email || "사용자",
      openclawAgentId,
    });
    ...
    const agent = await repository.createAgent({
      name,
      modelTier,
      systemPrompt: agentAbout,
      openclawAgentId,
      tools,
      skills,
      avatarUrl,
    });
    const openclawSync = syncAgentUpsertToOpenClawConfigs({
      openclawAgentId,
      name,
      tools: agent.tools,
      modelTier: agent.modelTier || modelTier,
      documents: profileDocs.documents,
    });
    const openclawCliSync = await syncAgentUpsertToOpenClawCli({
      openclawAgentId,
      workspacePath: Array.isArray(openclawSync?.workspacePaths)
        ? openclawSync.workspacePaths[0]
        : "",
      modelTier,
    });

    const defaultOpenclawAgentIds = await getOpenClawDefaultAgentIdSet();
    return res.status(201).json({
      agent: toAgentApiResponse(agent, {
        defaultOpenclawAgentIds,
      }),
      profileGeneration: profileDocs.generation,
      openclawSync,
      openclawCliSync,
    });
  } catch (error) {
...
app.put("/api/agents/:agentId", ...requireOperatorRole, async (req, res, next) => {
  try {
    const agentId = String(req.params.agentId || "").trim();
    if (!agentId) {
      return res.status(400).json({
    ...
    const openclawAgentId = resolveOpenClawAgentIdInput({
      openclawAgentId: req.body?.openclawAgentId,
      name,
      existingOpenclawAgentId: existingAgent?.openclawAgentId,
    });
    const avatarUrl = String(req.body?.avatarUrl || "").trim() || null;
    const profileDocs = await generateAgentProfileDocuments({
      agentName: name,
    ...
    if (typeof repository.updateAgent !== "function") {
      return res.status(501).json({
        error: "agent_update_not_supported",
        message: "agent update is not supported by current repository",
      });
    }

    const updated = await repository.updateAgent(agentId, {
      name,
      modelTier,
      systemPrompt: agentAbout,
      openclawAgentId,
    ...
    const openclawSync = syncAgentUpsertToOpenClawConfigs({
      ...
      tools: updated.tools,
      modelTier: updated.modelTier || modelTier,
      documents: profileDocs.documents,
    });
    const openclawCliSync = await syncAgentUpsertToOpenClawCli({
      openclawAgentId: updated.openclawAgentId || openclawAgentId,
    ...
    const previousOpenclawAgentId = String(existingAgent?.openclawAgentId || "").trim();
    const currentOpenclawAgentId = String(updated?.openclawAgentId || "").trim();
    const openclawRemovalSync =
      previousOpenclawAgentId &&
      currentOpenclawAgentId &&
      normalizeOpenClawAgentMatchToken(previousOpenclawAgentId) !==
        normalizeOpenClawAgentMatchToken(currentOpenclawAgentId)
        ? syncAgentRemovalToOpenClawConfigs({
            agentId: existingAgent.id,
            openclawAgentId: previousOpenclawAgentId,
            name: existingAgent.name,
          })
        : null;
    const openclawCliRemovalSync =
      previousOpenclawAgentId &&
      currentOpenclawAgentId &&
      normalizeOpenClawAgentMatchToken(previousOpenclawAgentId) !==
        normalizeOpenClawAgentMatchToken(currentOpenclawAgentId)
        ? await syncAgentRemovalToOpenClawCli({
            openclawAgentId: previousOpenclawAgentId,
          })
        : null;
    ...
    const defaultOpenclawAgentIds = await getOpenClawDefaultAgentIdSet();
    return res.json({
      agent: toAgentApiResponse(updated, {
        defaultOpenclawAgentIds,
      }),
      profileGeneration: profileDocs.generation,
      openclawSync,
...
module.exports = {
  ...
  ensureInitialized,
  toAgentApiResponse,
  resolveOpenClawAgentIdInput,
  removeAgentFromOpenClawConfigList,
  enforceMainDefaultInOpenClawList,
  isMainOpenClawAgentId,
  parseOpenClawCliJson,
  normalizeOpenClawAgentsListPayload,
  deriveOpenClawEffectiveToolList,
  buildOpenClawAgentToolsConfigFromSelection,
  upsertOpenClawAgentConfigListEntry,
  resolveOpenClawModelSelectionFromModelTier,
  applyOpenClawDefaultModelForMainAgent,
};
